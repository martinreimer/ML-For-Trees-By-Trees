# Merges climavi tree sensor data with openmeteo weather sensor data 
...
MERGED_DATA_PATH = os.path.join(DATA_PATH, "merged", "climavi_tree_openmeteo_weather")
...
WEATHER_DATA_PATH = os.path.join(RAW_DATA_PATH, "openmeteo", "Weather_Hourly.csv")
...
def merge_tree_w_weather_dfs(tree_data, weather_df):
    ...
    for tree_sensor_name, tree_df in tree_data.items():
        ...
        weather_df['datetime'] = weather_df['datetime'].dt.tz_localize(None) # remove timezone info

        tree_df.set_index('datetime', inplace=True)
        weather_df.set_index('datetime', inplace=True)
        
        # get first valid weather row index (has value for irradatation)
        first_valid_index = tree_df[tree_df['-10|ENV__SOIL__VWC'].notnull()].index[0]
        # Remove all rows before the first valid index
        weather_df = weather_df.loc[first_valid_index:]
        ...
        # save to csv
        tmp_path = os.path.abspath(os.path.join(MERGED_FINAL_DATA_PATH, f"{tree_sensor_name}_+_openmeteo_merged.csv"))
        df_left_merge.to_csv(tmp_path, index=False)
        print(f"Saved merged data to {tmp_path}")
        merged_data[f"{tree_sensor_name}_+_openmeteo"] = df_left_merge
    return merged_data
...
if __name__ == "__main__":
    ...
    df_sensors = df_sensor_table[df_sensor_table['merge_with_openmeteo_weather'] == True]
    print(f"To be merged Tree_Sensors:\n{df_sensors['label']}")

    # get data for favorite tree and weather sensors
    tree_data = get_data_for_sensor_type(df_sensors, TREE_DATA_PATH)
    weather_df = pd.read_csv(WEATHER_DATA_PATH, sep=',', encoding='latin1')

    # Preprocessing
    # loop over tree and weather data
    # loop over all dfs
    sensors_data = [tree_data]
    ...
    # Merge
    merge_tree_w_weather_dfs(sensors_data[0], weather_df)
    print("Done")